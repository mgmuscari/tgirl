...
2026-04-22: Claude Code agent definitions (`.claude/agents/*.md`) load their `tools:` frontmatter for team members even though the body prose does NOT load (refining 2026-03-09 gotcha on bug #24316 — the behavior is partial, not all-or-nothing). Team-member agents whose `tools:` line omits `SendMessage` and `TaskUpdate` will silently fail: they can read files but cannot communicate with teammates or claim/complete tasks, and the symptom is "agent goes idle after each prompt without taking action." → Every team-member agent definition in `.claude/agents/` MUST include `SendMessage, TaskCreate, TaskList, TaskGet, TaskUpdate` in its `tools:` line. Audit command: `grep -l '^tools:' .claude/agents/*.md | xargs grep -L 'SendMessage'` — should return nothing.
